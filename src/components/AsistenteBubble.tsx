...
export function AsistenteBubble({ hidden = false }: { hidden?: boolean }) {
  ...
  return (
    <>
      ...
      <div
        className={`pointer-events-none fixed right-3 md:right-6 bottom-[calc(4.85rem+env(safe-area-inset-bottom))] md:bottom-6 flex flex-col items-end gap-2 ${
          open ? 'z-[55]' : 'z-40'
        }`}
      >
        <AnimatePresence>
          {open && (
            <motion.section
              ...
              className="pointer-events-auto w-[min(20.5rem,calc(100vw-1.5rem))]"
            >
        ...
        {!open && !hidden && (
          <button
            ...
            className="pointer-events-auto h-11 w-11 md:h-12 md:w-12 rounded-full bg-navy dark:bg-mint text-mint dark:text-navy shadow-md shadow-navy/20 dark:shadow-mint/20 flex items-center justify-center border border-white/10 dark:border-transparent hover:scale-[1.03] active:scale-95 transition-transform"
          >
